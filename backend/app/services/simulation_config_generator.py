...
CHINA_TIMEZONE_CONFIG = {
    ...
    "activity_multipliers": {
        ...
        "morning": 0.4,    #Get active in the morning
...
class PlatformConfig:
    ...
    popularity_weight: float = 0.3  #Hot
...
"""
    ...
    ENTITIES_PER_TYPE_DISPLAY = 20       #Display the number of entities of each type
...
"""
        ...
        reasoning_parts.append(f"EnText: {time_config_result.get('reasoning', 'success')}")
        
        # ========== Step 2: Generate event configuration ==========
        report_progress(2, "Generating event configurations and hot topics...")
        event_config_result = self._generate_event_config(context, simulation_requirement, entities)
        event_config = self._parse_event_config(event_config_result)
        reasoning_parts.append(f"EnText: {event_config_result.get('reasoning', 'success')}")
...
- **Important**: The following example values XXare for reference only. You need to adjust the specific time period according to the nature of the event and the characteristics of the participating groups.
...
- **Time conforms to Chinese peopleXs schedule**: almost inactive from 0-5am in the morning, most active from 19-22pm in the evening
