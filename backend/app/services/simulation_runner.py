...
from .lightrag_memory_updater import GraphMemoryManager as ZepGraphMemoryManager
...
"""
    ...
    def start_simulation(
        ...
        graph_id: str = None  # Zep map ID (required when map update is enabled)
    ) -> SimulationRunState:
        """
Start simulation
...
"""
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError("graph_id must be provided when enabling graph memory updates")
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    if platform == "twitter":
                                        ...
                                        state.twitter_running = False
                                        logger.info(f"Twitter EnText: {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}")
                                    elif platform == "reddit":
                                        state.reddit_completed = True
                                        state.reddit_running = False
                                        logger.info(f"Reddit EnText: {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}")
...
"""
        ...
        if not os.path.exists(sim_dir):
            return {"success": True, "message": "The simulation directory does not exist, no need to clean"}
...
"""
        ...
        if not has_processes and not has_updaters:
            return  # If there is no content that needs to be cleaned, return silently.
            ...
            try:
                if process.poll() is None:  # The process is still running
                    ...
                    if state:
                        ...
                        state.reddit_running = False
                        state.completed_at = datetime.now().isoformat()
                        state.error = "The server is down and the simulation was terminated"
                        cls._save_run_state(state)
...
"""
        ...
        logger.info(f"EnTextInterviewEnText: simulation_id={simulation_id}, agent_id={agent_id}, platform={platform}")
