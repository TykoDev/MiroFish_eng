        ...
        if self.entity_insights:
            ...
            for entity in self.entity_insights:
                ...
                if entity.get('summary'):
                    text_parts.append(f"  EnText: \"{entity.get('summary')}\"")
...
"""
    ...
    def to_text(self) -> str:
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Current valid facts] (original simulation result)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                while clean_quote and clean_quote[0] in 'X,X;X:XXXX\n\r\t ':
...
"""
    ...
    def search_graph(
        ...
        try:
            ...
            logger.info(f"Search completed: {len(facts)} relevant facts found")
...
"""
        ...
        keywords = [w.strip() for w in query_lower.replace(',', ' ').replace('X', ' ').split() if len(w.strip()) > 1]
        ...
        try:
            ...
            logger.info(f"Local search completed: {len(facts)} relevant facts found")
...
edge list
"""
        logger.info(f"Get the relevant edges of node {node_uuid[:8]}...")
        ...
        try:
            ...
            logger.info(f"Find {len(result)} edges related to the node")
            return result
...
"""
        logger.info(f"Get the entity of type {entity_type}...")
...
"""
        logger.info(f"Get the relationship summary for entity {entity_name}...")
...
Statistics
"""
        logger.info(f"Get statistical information of graph {graph_id}...")
...
Returns a list of subquestions in JSON format. """
        ...
        try:
            ...
            logger.warning(f"Failed to generate subproblem: {str(e)}, use default subproblem")
    ...
    def panorama_search(
        ...
        """
XPanoramaSearch - Breadth SearchX
...
"""
        ...
        logger.info(f"PanoramaSearch completed: {result.active_count} valid items, {result.historical_count} historical items")
        return result
...
"""
    ...
    def interview_agents(
        ...
        """
XInterviewAgents - In-depth interviewX
...
"""
        ...
        if not profiles:
            logger.warning(f"The character file for simulation {simulation_id} was not found")
            result.summary = "No interviewable Agent profile found"
            return result
        
        result.total_agents = len(profiles)
        logger.info(f"Loaded into {len(profiles)} Agent profiles")
        ...
        logger.info(f"{len(selected_agents)} Agents were selected for interviews: {selected_indices}")
        ...
        if not result.interview_questions:
            ...
            logger.info(f"{len(result.interview_questions)} interview questions generated")
        ...
        try:
            ...
            api_result = SimulationRunner.interview_agents_batch(
                ...
                platform=None,  # Without specifying platform, dual-platform interview
            ...
            if not api_result.get("success", False):
                ...
                logger.warning(f"Interview API failed to return: {error_msg}")
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                response_text = f"XTwitter platform answerX\n{twitter_text}\n\nXReddit platform answerX\n{reddit_text}"
        ...
        logger.info(f"InterviewAgents completed: interviewed {result.interviewed_count} Agents (dual platforms)")
        return result
    ...
    def _select_agents_for_interview(
        ...
        try:
            ...
            logger.warning(f"LLM failed to select Agent, use the default selection: {e}")
