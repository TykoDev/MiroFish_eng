        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(
                        f"Function {func.__name__} {attempt + 1} failed: {str(e)}, "
                        f"Retry in {current_delay:.1f} seconds..."
                    )
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    logger.warning(
                        f"The asynchronous function {func.__name__} failed at the {attempt + 1}th time: {str(e)}, "
                        f"Retry in {current_delay:.1f} seconds..."
                    )
...
"""
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                logger.warning(
                    f"API call failed at {attempt + 1}th time: {str(e)}, "
                    f"Retry in {current_delay:.1f} seconds..."
                )
