...
"""
        try:
            ...
            error_msg = str(e)
            print(f"  InterviewEnText: agent_id={agent_id}, error={error_msg}")
...
"""
        ...
        print(f"LLMEnText: model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else 'default'}...")
...
"""
        ...
        # Calculate the activation probability based on the configuration of each Agent
        candidates = []
...
"""
        ...
        print(f"EnTextID: {self.config.get('simulation_id', 'unknown')}")
        ...
        print(f"  - AgentEnText: {len(self.config.get('agent_configs', []))}")
        ...
        # Create environment
        print("Creating an OASIS environment...")
        self.env = oasis.make(
        ...
        if initial_posts:
            ...
            if initial_actions:
                ...
                print(f"{len(initial_actions)} initial posts have been posted")
...
"""
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            ...
            # Forced to exit after receiving repeated signals
            print("Force quit...")
            sys.exit(1)
