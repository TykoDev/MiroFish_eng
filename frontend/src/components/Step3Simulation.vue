...
<!-- Universal fallback: unknown type or content but not processed above -->
...
maxRounds: Number, // The maximum number of rounds passed in from Step2
  minutesPerRound: {
    type: Number,
default: 30 //The default is 30 minutes per round
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
stopPolling() // Stop possible previous polling
}
...
addLog('✓ Old simulation logs have been cleared and simulation has been restarted')
      }
addLog('✓ The simulation engine started successfully')
...
// Detect round changes on each platform and output logs
...
// Check whether the simulation has been completed (judged by runner_status or platform completion status)
...
addLog('✓ All platform simulations detected have ended')
        }
...
// Determine whether the platform is enabled through actions_count (if count > 0 or running was true)
...
console.warn('Failed to obtain detailed status:', err)
  }
}
