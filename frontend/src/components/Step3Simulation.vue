    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? 'XXX...' : 'XXXXXXXX' }}
          <span v-if="!isGeneratingReport" class="arrow-icon">X</span>
        </button>
      </div>
    </div>
          ...
          >
            ...
            <div class="timeline-card">
              ...
              <div class="card-footer">
                <span class="time-tag">R{{ action.round_num }} X {{ formatActionTime(action.timestamp) }}</span>
...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // EN_Comment
  minutesPerRound: {
    type: Number,
    default: 30 // EN_Comment
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const runStatus = ref({})
const allActions = ref([]) // EN_Comment
const actionIds = ref(new Set()) // EN_Comment
...
const doStartSimulation = async () => {
  ...
  try {
    const params = {
      ...
      platform: 'parallel',
      force: true,  // EN_Comment
      enable_graph_memory_update: true  // EN_Comment
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`XXXXXXXX: ${props.maxRounds}`)
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('X XXXXXXXXXXXXXXXX')
      }
      addLog('X XXXXXXXX')
      addLog(`  XX PID: ${res.data.process_pid || '-'}`)
      ...
      startError.value = res.error || 'XXXX'
      addLog(`X XXXX: ${res.error || 'Unknown error'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`X XXXX: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`X XXXXXXXXX: ${reportId}`)
      
      // EN_Comment
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`X XXXXXXXX: ${res.error || 'Unknown error'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`X XXXXXXXX: ${err.message}`)
    isGeneratingReport.value = false
  }
}
