<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Левая панель: граф в реальном времени -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          ...
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? 'Выйти из полноэкранного' : 'Полный экран'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">После завершения генерации автоматически начнётся построение графа</p>
          </div>
          
          <!-- Строится, но данных ещё нет -->
            ...
            <div class="phase-detail">
              <div class="detail-section">
                ...
                <div class="detail-content">
                  После загрузки документов LLM анализирует их содержимое и автоматически генерирует онтологическую структуру для симуляции (типы сущностей + типы связей)
                </div>
              </div>
              ...
              <!-- Информация о сгенерированной онтологии -->
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} связей...
                  </div>
                </div>
              </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">Описание API</div>
                <div class="detail-content">
                  На основе онтологии документы разбиваются на фрагменты и передаются в Zep API для построения графа знаний с извлечением сущностей и связей
                </div>
              </div>
              
              <!-- Ожидание завершения онтологии -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">Ожидание завершения онтологии...</div>
              </div>
              ...
              <div class="detail-section" v-if="graphData">
                <div class="detail-label">Результаты построения</div>
...
const currentPhase = ref(-1) // -1: загрузка, 0: онтология, 1: граф, 2: завершено
const selectedItem = ref(null) // Выбранный узел или ребро
...
const goToNextStep = () => {
  // TODO: переход к следующему шагу
...
const initProject = async () => {
  ...
  if (paramProjectId === 'new') {
    // Новый проект: берём данные из store
...
// Новый проект - вызов ontology/generate
const handleNewProject = async () => {
  const pending = getPendingUpload()
  
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'Нет файлов, вернитесь на главную'
    loading.value = false
    return
  }
  ...
  try {
    loading.value = true
    currentPhase.value = 0 // Этап онтологии
    ontologyProgress.value = { message: 'Загрузка и анализ...' }
    ...
    if (response.success) {
      ...
      // Автозапуск построения графа
      await startBuildGraph()
    } else {
      error.value = response.error || 'Ошибка онтологии'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'Ошибка инициализации: ' + (err.message || 'Неизвестно')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || 'Ошибка'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      buildProgress.value.message = 'Задача запущена...'
      ...
      error.value = response.error || 'Ошибка запуска'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'Ошибка: ' + (err.message || 'Неизвестно')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('✅ Граф построен, загружаем данные...')
        ...
        buildProgress.value = {
          progress: 100,
          message: 'Построено, загружаем граф...'
        }
        
        // Перезагружаем для graph_id
        const projectResponse = await getProject(currentProjectId.value)
        if (projectResponse.success) {
          ...
          if (projectResponse.data.graph_id) {
            console.log('📊 Граф:', projectResponse.data.graph_id)
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'Ошибка: ' + (task.error || 'Неизвестно')
        buildProgress.value = null
      }
    }
  } catch (err) {
