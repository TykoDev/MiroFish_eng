...
const toggleSectionCollapse = (idx) => {
  // Сворачиваем только завершённые разделы
  if (!generatedSections.value[idx + 1]) return
...
const toolConfig = {
  'insight_forge': {
    name: 'Deep Insight',
    color: 'purple',
    icon: 'lightbulb' // Инсайты
  },
  'panorama_search': {
    name: 'Panorama Search',
    color: 'blue',
    icon: 'globe' // Глобальный поиск
  },
  'interview_agents': {
  ...
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
    icon: 'zap' // Быстрый доступ
  },
  'get_graph_statistics': {
    name: 'Graph Stats',
    color: 'cyan',
    icon: 'chart' // Статистика
  },
  'get_entities_by_type': {
    name: 'Entity Query',
    color: 'pink',
    icon: 'database' // Сущности
  }
}
...
const parseInsightForge = (text) => {
  ...
  try {
    // Извлечение аналитического запроса
    const queryMatch = text.match(/Аналитический запрос:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    
    // Извлечение сценария прогнозирования
    const reqMatch = text.match(/Сценарий прогнозирования:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = reqMatch[1].trim()
    
    // Извлечение статистики
    const factMatch = text.match(/Связанные факты:\s*(\d+)/)
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Извлечение статистики
    const nodesMatch = text.match(/Узлов:\s*(\d+)/)
    const edgesMatch = text.match(/Рёбер:\s*(\d+)/)
    ...
    // Извлечение активных фактов
    const activeSection = text.match(/### 【Текущие активные факты】[\s\S]*?\n([\s\S]*?)(?=\n###|$)/)
    if (activeSection) {
    ...
    // Извлечение исторических фактов
...
const parseInterview = (text) => {
  ...
  try {
    ...
    // Извлечение числа опрошенных
    const countMatch = text.match(/\*\*Количество опрошенных:\*\*\s*(\d+)\s*\/\s*(\d+)/)
    if (countMatch) {
    ...
    const reasonMatch = text.match(/### Причины выбора респондентов\n([\s\S]*?)(?=\n---\n|\n### Записи интервью)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // Формат 1: номер. **имя(index=X)**: причина
        // Пример: 1. **alumni_345(index=1)**: ...
        ...
        // Формат 2: - имя(index X): причина
        // Пример: - parent_601(index 0): ...
        if (!headerMatch) {
        ...
        if (name) {
          // Сохраняем предыдущее обоснование
      ...
      // Сохраняем последнее обоснование
      if (currentName && currentReason.length > 0) {
        reasons[currentName] = currentReason.join(' ').trim()
      }
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      // Извлечение ответов по платформам
      const answerMatch = block.match(/\*\*A:\*\*\s*([\s\S]*?)(?=\*\*关键引言|$)/)
      if (answerMatch) {
        const answerText = answerMatch[1].trim()
        
        // Разделяем Twitter и Reddit
      ...
      if (quotesMatch) {
        ...
        // Фолбэк: > "text" или > “text” (китайские кавычки)
        if (!quoteMatches) {
    ...
    const summaryMatch = text.match(/### Итоги интервью и ключевые выводы\n([\s\S]*?)$/)
    if (summaryMatch) {
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    const queryMatch = text.match(/ПоискЗапрос:\s*(.+?)(?:\n|$)/)
    if (queryMatch) result.query = queryMatch[1].trim()
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-header' }, [
        ...
        props.result.simulationRequirement && h('div', { class: 'header-scenario' }, [
          h('span', { class: 'scenario-label' }, 'Сценарий прогнозирования: '),
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Ключевые факты из памяти'),
            h('span', { class: 'panel-count' }, `${props.result.facts.length} записей`)
          ]),
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.entities.length} шт.`)
          ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.relations.length} записей`)
          ]),
        ...
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Подвопросы дрифт-анализа'),
            h('span', { class: 'panel-count' }, `${props.result.subQueries.length} шт.`)
          ]),
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.activeFacts.length} записей`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет данныхТекущая активная память'),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.historicalFacts.length} записей`)
          ]),
          ...
          ) : h('div', { class: 'empty-state' }, 'Нет данныхИсторическая память'),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Проверка заглушки платформы
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '（该平台未获得回复）' || t === '(该平台未获得回复)' || t === '[无回复]' || t === '(no reply)'
    }

    // Разбиение по номерам
    ...
    const getAnswerForQuestion = (interview, qIdx, platform) => {
      ...
      // Fallback: полный ответ
      return qIdx === 0 ? answer : ''
    }
    
    // Проверка двух платформ
    const hasMultiplePlatforms = (interview, qIdx) => {
      ...
      // Обе платформы с разными ответами
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Кнопки переключения платформ (только если обе дали ответ)
    ...
    return () => h('div', { class: 'quick-search-display' }, [
      ...
      h('div', { class: ['quicksearch-content', { 'no-tabs': !showTabs.value }] }, [
        ...
        ((!showTabs.value) || activeTab.value === 'facts') && h('div', { class: 'facts-panel' }, [
          ...
          ) : h('div', { class: 'empty-state' }, 'Результатов не найдено'),
          props.result.facts.length > INITIAL_SHOW_COUNT && h('button', {
            class: 'expand-btn',
            onClick: () => { expandedFacts.value = !expandedFacts.value }
          }, expandedFacts.value ? `Свернуть ▲` : `Показать все ${props.result.facts.length} ▼`)
        ...
        activeTab.value === 'edges' && hasEdges.value && h('div', { class: 'edges-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.edges.length} записей`)
          ]),
        ...
        activeTab.value === 'nodes' && hasNodes.value && h('div', { class: 'nodes-panel' }, [
          h('div', { class: 'panel-header' }, [
            ...
            h('span', { class: 'panel-count' }, `${props.result.nodes.length} шт.`)
          ]),
...
const activeStep = computed(() => {
  ...
  // Если нет active - последний done
...
const renderMarkdown = (content) => {
  ...
  // Убираем заголовок ## (уже показан в обёртке)
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            currentSectionIndex.value = null  // Очищаем loading
        ...
        nextTick(() => {
          if (rightPanel.value) {
            // При завершении - наверх, иначе - вниз
            if (isComplete.value) {
...
// Извлечение финального ответа из LLM
