...
const props = defineProps({
  simulationId: String,
  maxRounds: Number, // Максимум раундов из Step2
  minutesPerRound: {
    type: Number,
    default: 30 // По умолчанию 30 мин/раунд
  },
  projectData: Object,
  graphData: Object,
  systemLogs: Array
...
const phase = ref(0) // 0: не начато, 1: запущено, 2: завершено
const isStarting = ref(false)
const isStopping = ref(false)
const startError = ref(null)
const runStatus = ref({})
const allActions = ref([]) // Все действия
const actionIds = ref(new Set()) // Для дедупликации
...
const resetAllState = () => {
  ...
  stopPolling()  // Останавливаем опрос
}

// Запуск симуляции
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog('Ошибка: нет simulationId')
    return
  }
  
  // Сбрасываем для чистого старта
  resetAllState()
  
  isStarting.value = true
  startError.value = null
  addLog('Запуск параллельной симуляции...')
  ...
  try {
    const params = {
      ...
      enable_graph_memory_update: true  // Динамическое обновление графа
    }
    
    if (props.maxRounds) {
      params.max_rounds = props.maxRounds
      addLog(`Раундов: ${props.maxRounds}`)
    }
    
    addLog('Режим обновления графа включён')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ Очищены старые логи, перезапуск')
      }
      ...
      addLog(`✗ Ошибка: ${res.error || 'Неизвестно'}`)
...
// Отслеживаем последний раунд по платформам
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Изменения раундов по платформам
      ...
      // Проверяем статус платформ
      // По флагам twitter_completed / reddit_completed
      const platformsCompleted = checkPlatformsCompleted(data)
      
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ Все платформы завершены')
        }
    ...
    console.warn('Ошибка статуса:', err)
  }
}

// Проверка завершения всех платформ
const checkPlatformsCompleted = (data) => {
  ...
  // Если хоть одна завершена
  // Активность платформы по actions_count
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('Ошибка деталей:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ Задача запущена: ${reportId}`)
