<template>
  ...
  >
    <!-- Фоновая сетка (только при наличии проектов) -->
    ...
    <!-- Карточки (только при наличии проектов) -->
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                ...
                <div class="modal-empty" v-else>Нет файлов</div>
              </div>
            </div>

            <!-- Разделитель воспроизведения -->
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-text">Граф знаний</span>
              </button>
              <button 
            ...
            <!-- Подсказка: недоступно для воспроизведения -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 «Запуск симуляции» и Step5 «Глубокое взаимодействие» доступны только в активном режиме — воспроизведение не поддерживается</span>
            </div>
          </div>
        </div>
      </Transition>
...
const selectedProject = ref(null)  // Выбранный проект (для модалки)
let observer = null
let isAnimating = false  // Блокировка анимации
let expandDebounceTimer = null  // Таймер дебаунса
let pendingState = null  // Ожидаемое состояние

// Конфигурация раскладки карточек
...
const containerStyle = computed(() => {
  if (!isExpanded.value) {
    // Свёрнуто: фиксированная высота
    return { minHeight: '420px' }
  }
  
  // Развёрнуто: высота по кол-ву карточек
  ...
  // Высота = строки * высота карточки + отступы
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Сброс таймера дебаунса
        if (expandDebounceTimer) {
          clearTimeout(expandDebounceTimer)
          expandDebounceTimer = null
        }
        
        // Если анимация - только запоминаем
        if (isAnimating) return
        
        // Одинаковое состояние - пропускаем
        if (shouldExpand === isExpanded.value) {
          pendingState = null
          return
        }
        
        // Дебаунс переключения
        // Задержка: 50ms раскрытие, 200ms скрытие
    ...
    {
      // Несколько порогов для плавности
      threshold: [0.4, 0.6, 0.8],
      // Корректируем rootMargin
...
// При keep-alive: перезагрузка при активации
onActivated(() => {
  loadHistory()
})
...
.cards-container {
  ...
  /* min-height задаётся динамически в JS */
}
...
/* Цвета по функциям */
.status-icon:nth-child(1).available { color: #3B82F6; } /* Граф - синий */
.status-icon:nth-child(2).available { color: #F59E0B; } /* Среда - оранжевый */
.status-icon:nth-child(3).available { color: #10B981; } /* Отчёт - зелёный */
...
/* Цвета прогресса */
.card-progress.completed { color: #10B981; }    /* Завершено */
.card-progress.in-progress { color: #F59E0B; }  /* В процессе */
.card-progress.not-started { color: #9CA3AF; }  /* Не начато */
